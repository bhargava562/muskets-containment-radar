...
function buildCaseFromAlert(alert) {
  const amount = Number(alert.totalAmount || 0)
  
  // Calibrate priority based on RBI/PMLA guidelines:
  // RBI Master Direction KYC Para 38(ii) mandatory STR threshold is ₹1,0,000 for P1
  const priority = amount >= 100000 ? 'P1' : (amount >= 50000 ? 'P2' : 'P3')
  
  // Indian naming base for realistic look
  const customerNames = [
    "Rajesh Chandra Sekhar", 
    "Sanjay Deshmukh", 
    "Sunita Patil", 
    "Amit Kumar Verma", 
    "Meenakshi Sundaram"
  ...
  return {
    ...
    priority: priority,
    riskAmount: amount,
    tracedAmount: Math.round(amount * 0.75), // typical recovery target
    totalBalance: Math.round(amount * 1.8),
    status: CASE_STATUS.PENDING_TRIAGE,
    aiSummary: `₹${(amount / 100000).toFixed(2)}L MuleHunter alert triggered. Velocity: ${alert.velocity} tx/min, Layering Intensity (FR): ${alert.fragRatio}. Accelerated transaction speed indicates active mule relay.`,
    graphId: 'graph_fragmentation',
    victimAccount: "185501000012847",
    muleAccounts: ["185502000087321"],
    merchantAccounts: ["916010000045678"],
    timestamp: alert.timestamp || new Date().toISOString(),
    analystName: "",
    investigatorNotes: "",
    auditLog: [
      { actor: "System", action: "Alert generated by anomaly detection engine (MuleHunter)", timestamp: alert.timestamp || new Date().toISOString() },
      { actor: "System", action: `Triage signals computed: FR=${alert.fragRatio}, Velocity=${alert.velocity} tx/min`, timestamp: alert.timestamp || new Date().toISOString() }
    ],
    alternateAISummaries: [
      `[ENHANCED DEPTH SCAN] MuleHunter flag triggered. Velocity: ${alert.velocity} tx/min indicates active layering network. Fragmentation Ratio: ${alert.fragRatio} matches standard structuring limits.`,
      `[ADVANCED RE-SCAN] Composite risk score: ${alert.riskScore}/100. High probability of mule relay cluster. Recommending Full Freeze or Proportional Lien based on triage signals.`
    ]
  }
}
